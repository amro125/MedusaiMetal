...
def draw_flow(img, flow, step=16):
    ...
    for (x1, y1), (x2, y2) in lines:
        arrows.append(math.sqrt((x2-x1)*(x2-x1) + (y2-y1)*(y2-y1)))
...
def scaleDiff(value,lastV,min,max):
    scaled_value = int(((lastV-value) - min) / (max - min) * 10)
    return scaled_value
...
if __name__ == '__main__':
    ...
    while True:
        ...
        movingaverage.append(arrows)
        avg = np.sum(movingaverage)/len(movingaverage)
        scaled = abs(scaleDiff(avg,lastavg,minMove,maxMove))
        if scaled != lastMove:
            print(scaled)
